...
type LocationQuery struct {
	...
	withNamedChildren map[string]*LocationQuery
...
func (lq *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	for name, query := range lq.withNamedChildren {
		if err := lq.loadChildren(ctx, query, nodes,
			func(n *Location) { n.appendNamedChildren(name) },
			func(n *Location, e *Location) { n.appendNamedChildren(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (lq *LocationQuery) WithNamedChildren(name string, opts ...func(*LocationQuery)) *LocationQuery {
	query := (&LocationClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if lq.withNamedChildren == nil {
		lq.withNamedChildren = make(map[string]*LocationQuery)
	}
	lq.withNamedChildren[name] = query
	return lq
}
